...
function calc_1d_aprox_iter(func, level)
	...
	for lev in 0:level
		...
		for i in min(1,lev):min(2,lev+1):n_lev
			...
			if lev>0
				surps[id] -= (func((i-1)*hn_lev) + func((i+1)*hn_lev)) * .5
				quad += surps[id] * hn_lev
			end
...
# Recursive method
function calc_1d_level(func, max_level, lev, aprox, surps, quad)
	if lev > max_level
		return quad
	end
	...
	return calc_1d_level(func, max_level, lev+1, aprox, surps, quad)
end

# ╔═╡ fdb10ff2-82f4-4431-8d3e-679b0800e5e3
# Recursive method
function calc_1d_aprox_rec(func, max_level)
	n_1d = 2^max_level+1
	aprox = zeros(plot_len)
	surps = ones(2^max_level+1)
	quad = (func(0)+func(1))*.5

	quad = calc_1d_level(func, max_level, 0, aprox, surps, quad)
	return aprox, surps, quad
end

# ╔═╡ 554f531a-6b27-401f-a58e-4a7d6f5817d7
begin
	aprox_1d, surps_1d, quad_1d = calc_1d_aprox_rec(func_1d, level_1d)
	fig_1d_aprox = Figure()
...
begin
	func_v_2d(x) = func_2d(x[1],x[2])
	sol_2d = hcubature(func_v_2d, [0,0], [1,1])[1]
	err_2d = abs(sol_2d-quad_2d)/sol_2d
	md"""
	The value of the integral is **$( round(sol_2d, digits=5) )**
	
	The approximated value with **$(2^level_2d+1)** points is **$( round(quad_2d, digits=5) )**
